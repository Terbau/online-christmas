      ...
      case "down-all": {
        ...
        do {
          ...
          if (res.results && !res.error) {
            res.results.forEach((r) => console.log(`${r.direction} ${r.migrationName}: ${r.status}`))
          } else {
            break
          }
        ...
        if (res.error) {
          console.log(`Failed to down all in migration "${res.results?.[0].migrationName}": ${res.error}`)
        }
        break
      }
      case "latest": {
